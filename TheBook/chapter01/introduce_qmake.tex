...
\begin{itemize}
\item 第1{\sourcefontone\~{}}2行表示不使用Qt库；
\item 第4行表示这是一个控制台应用程序；
\item 第6{\sourcefontone\~{}}10行表示在debug模式下输出目标名称是“hellow\underline{\hspace{0.5em}}world\underline{\hspace{0.5em}}debug”，
在release模式下输出目标名称是“hellow\underline{\hspace{0.5em}}world”；
\item 第12行表示输出的是一个应用程序；
\item 第14{\sourcefontone\~{}}18行表示使用C{\sourcefontone{}+}{\sourcefontone{}+}17标准；
...
不难发现qmake的语法十分简单：
\begin{itemize}
\item “{\sourcefontone{}=}”代表赋值；
\item “{\sourcefontone{}+}{\sourcefontone{}=}”代表向变量中增加元素；
\item “{\sourcefontone{}-}{\sourcefontone{}=}”代表从变量中删除元素；
\item “{\sourcefontone{}*}{\sourcefontone{}=}”代表如果变量中不存在则加入元素否则忽略；
\item “{\sourcefontone\~{}}{\sourcefontone{}=}”代表替换变量中的值；
\item “{\sourcefontone\$}{\sourcefontone\$}”代表当qmake运行时，变量的字面值；
\item “{\sourcefontone\$}”代表当qmake生成Makefile后，变量的字面值；
\item “{\sourcefontone\#}”代表注释；
\item “SOURCES”代表需要编译的C{\sourcefontone{}/}C{\sourcefontone{}+}{\sourcefontone{}+}源代码变量；
\item “HEADERS”代表C{\sourcefontone{}/}C{\sourcefontone{}+}{\sourcefontone{}+}头文件变量；
\item “DEFINES”代表C{\sourcefontone{}/}C{\sourcefontone{}+}{\sourcefontone{}+}宏变量；
...
第6{\sourcefontone\~{}}10行和14{\sourcefontone\~{}}18虽然写法不同，实际上都是检查“CONFIG”中是否定义了特定项。
...
并要求它们按照一定先后顺序编译。
比如本节采用的“CONFIG {\sourcefontone{}+}{\sourcefontone{}=} ordered”就要求项目按照定义顺序编译。
...
比起上一节，它多了一些新的知识点。
\begin{itemize}
\item 第21{\sourcefontone\~{}}23行更改了在非Windows平台下程序的链接参数，
它要求程序运行时将其所在目录加入动态库搜索路径；
\item 第28行将另一个文件引入此文件，它和C{\sourcefontone{}/}C{\sourcefontone{}+}{\sourcefontone{}+}的“{\sourcefontone\#}include”工作原理一致；
\end{itemize}
...
它也引入了一些新的知识。
\begin{itemize}
\item 第2行使用“INCLUDEPATH”变量将当前目录加入C{\sourcefontone{}/}C{\sourcefontone{}+}{\sourcefontone{}+}包含路径搜索路径；
\item 第3{\sourcefontone\~{}}7使用“LIBS”变量导入C{\sourcefontone{}/}C{\sourcefontone{}+}{\sourcefontone{}+}链接库，
“{\sourcefontone{}-}L”后面是库所在路径，
“{\sourcefontone{}-}l”后面紧跟库的名称；
\end{itemize}
%\begin{spacing}{1.0}
...
一切与定义应用程序没什么不同，只是将
“TEMPLATE {\sourcefontone{}=} app”改成了
“TEMPLATE {\sourcefontone{}=} lib”，如\lstlistingname\ \ref{f000012} 第13行所示。
...
。
都是标准C{\sourcefontone{}+}{\sourcefontone{}+}，本书不赘述。
%\begin{spacing}{1.0}
...
\caption{qmake对C{\sourcefontone{}/}C{\sourcefontone{}+}{\sourcefontone{}+}编译链接过程控制点} %标题
\label{p000002} %索引
\end{figure}


如\figurename\ \ref{p000002}
一个C{\sourcefontone{}/}C{\sourcefontone{}+}{\sourcefontone{}+}程序编译至少可以抽象出三个节点，
源代码编译前，
链接前以及链接后。
这三个时刻分别对应于qmake变量：
...
\begin{itemize}
\item 第28{\sourcefontone\~{}}41行展示了如何使用
“QMAKE\underline{\hspace{0.5em}}EXTRA\underline{\hspace{0.5em}}COMPILERS”。
...
读者可以用此技术实现自定义代码生成器，
不过这需要读者有编译原理相关知识。
\item 第44{\sourcefontone\~{}}49行展示了如何使用
...
\item 第52{\sourcefontone\~{}}57行展示了如何使用
“QMAKE\underline{\hspace{0.5em}}POST\underline{\hspace{0.5em}}LINK”。
...
而利用C{\sourcefontone{}+}{\sourcefontone{}+} 17的filesystem模块自己实现一个单纯的拷贝程序并不复杂。
...
、“before\underline{\hspace{0.5em}}run{\sourcefontone{}/}main.cpp”
（如\lstlistingname\ \ref{f000009}）
、“after\underline{\hspace{0.5em}}run.pro”
（如\lstlistingname\ \ref{f000006}）
、“after\underline{\hspace{0.5em}}run{\sourcefontone{}/}main.cpp”
（如\lstlistingname\ \ref{f000007}）
、“new\underline{\hspace{0.5em}}moc.pro”
（如\lstlistingname\ \ref{f00000b}）
、“new\underline{\hspace{0.5em}}moc{\sourcefontone{}/}main.cpp”
（如\lstlistingname\ \ref{f00000c}）
和
“the\underline{\hspace{0.5em}}run{\sourcefontone{}/}main.cpp”
...
其核心指令只有一条：
    ...
    \begin{center}
        \sourcefontone{}qmake {\sourcefontone{}-}r {\sourcefontone{}-}tp vc <工程名称> 
    \end{center}
