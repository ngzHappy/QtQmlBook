#include <sstd_qt_and_qml_library.hpp>
#include "MineSweepingWindow.hpp"

int main(int argc, char ** argv) {

    /*初始化程序*/
    auto varApp = sstd_make_unique< sstd::Application >(argc, argv);
    auto varMineSweepingWindow = sstd_make_unique<MineSweepingWindow>();
    {
        /*初始化Qml/Quick引擎*/
        auto varWindow = sstd_new< sstd::DefaultRoowWindow >();
        {
            /*获得Qml文件绝对路径*/
            auto varFullFileName = sstd::getLocalFileFullPath(
                QStringLiteral("myqml/minesweeping/main.qml"));
            /*加载Qml文件*/
            varWindow->load(varFullFileName);
            /*检查并报错*/
            if (varWindow->status() != sstd::LoadState::Ready) {
                qWarning() << QStringLiteral("can not load : ") << varFullFileName;
                return -1;
            }
        }
        varMineSweepingWindow->setWineSweepingWindow(varWindow);
    }
    varMineSweepingWindow->show();

    return varApp->exec();

}







